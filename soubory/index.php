		...
		<div class="container">
			...
			<div class="kategorie">
				...
				<div class="tooltip">zobraz si volné produkty
								...
								FROM `mp_evidence_produktu`, `vypujcka-produkty` `vypujcka_produkty`, `mp_vypujcka`, `mp_produkty`
								WHERE `mp_evidence_produktu.id` = `vypujcka_produkty.id_technika` and `mp_vypujcka.id` = `vypujcka_produkty.id_vypujcka` and `mp_produkty.ID` = '$id'";
								$result2 = mysqli_query($connect, $query2);
								if(mysqli_num_rows($result2) > 0){
						
									while($row2= mysqli_fetch_array($result2)){								   
										echo ("Od:".$row2["mp_vypujcka.od"]." Do:".$row2["mp_vypujcka.do"]) ;
										}
								}
								else{echo "tento produkt je volný";}
